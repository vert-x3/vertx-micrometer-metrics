...
The Vert.x SQL client expose <<clients-metrics,client metrics>> as well as <<pool-metrics,pool metrics>> both use the `sql` type.
...
When using several pools, you can set a pool name on the `PoolOptions`, exposed as a label.
...
Likewise, you can set the client namespace on the `SqlConnectOptions`, exposed as a label
