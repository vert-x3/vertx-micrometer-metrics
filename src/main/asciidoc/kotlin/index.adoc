...
router.route("/metrics").handler({ routingContext ->
  var prometheusRegistry = io.vertx.micrometer.backend.BackendRegistries.getDefaultNow()
...
[source,kotlin]
----
var registry = io.vertx.micrometer.backend.BackendRegistries.getDefaultNow()
...
// Later on:
var registry = io.vertx.micrometer.backend.BackendRegistries.getNow("my registry")
