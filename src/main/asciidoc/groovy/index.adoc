...
* Vert.x core tools micrometer: TCP/HTTP client and servers, `link:../../apidocs/io/vertx/core/datagram/DatagramSocket.html[DatagramSocket]`,
...
router.route("/metrics").handler({ routingContext ->
  def prometheusRegistry = io.vertx.micrometer.backend.BackendRegistries.getDefaultNow()
...
[source,groovy]
----
def registry = io.vertx.micrometer.backend.BackendRegistries.getDefaultNow()
...
// Later on:
def registry = io.vertx.micrometer.backend.BackendRegistries.getNow("my registry")
...
A `link:../../apidocs/io/vertx/micrometer/service/MetricsService.html[MetricsService]` can be created out of a `link:../../apidocs/io/vertx/core/metrics/Measured.html[Measured]` object
